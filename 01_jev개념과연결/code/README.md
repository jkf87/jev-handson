# 1강 실습 코드 — 녹화 실습(jev-handson) + 더 해 보기

**강의 실습은 `jev-handson/`이에요.** 2026-09-23 녹화에서 Codex 데스크톱 앱에 프롬프트 네 개(`../프롬프트.md` ①~④)를 넣어 만든 결과물 그대로예요. OpenRouter(`~typesafe/jev-latest`) 키 한 줄을 `jev-handson/env.txt`에 넣고, 에이전트에게는 파일 경로만 알려 줘요. 실행법과 녹화 결과: `jev-handson/README.md`

```bash
cd jev-handson && npm install
npm start                 # example.js: 질문 세 개(noul · choice · score)를 한 요청에
npm run examples          # three-examples.js: 셋을 따로
cd benchmark-site && npm install && npm run sample && npm start   # RouteLab http://127.0.0.1:4173 (키 없어도 기준값 화면은 보임)
```

아래는 강의에서 다루지 않는 **더 해 보기**예요(프롬프트는 `../프롬프트.md` 「더 해 보기」 A~C). TypeSafe 키로 직접 부르고, 471건 전부로 LLM과 비교하고, 에이전트에 스킬 · 훅 · MCP로 붙여 봐요. 9/23에 Claude Code(`claude -p`) · Codex로 그대로 돌린 기록: `../evidence/prompts/`

## 더 해 보기 A · TypeSafe 키로 직접 부르기
...
python3 first_call.py                    # 파이썬 표준 라이브러리 예제 (프롬프트 1.1의 예제 답안)
```
...
## 더 해 보기 B · 471건 전부로 LLM과 Jev 비교 — `04_악플탐지/`
...
### 스킬 설치
```bash
# 프롬프트로 할 때는 demo-project 안에서 그대로 합니다(../프롬프트.md 2.1 · 2.2). 직접 칠 때는 복사해서:
...
### '쓸 수 있음'과 '반드시 씀'은 다르다 — 훅 · Codex · MCP (더 해 보기 C 이어서)
프롬프트는 `../프롬프트.md` 「더 해 보기」 C(훅.1~3 · codex.1~2 · mcp.1~2)에 있어요.
...
- MCP로 붙이기: `jev-mcp/jev_mcp_server.py` (도구 `jev_decide`, `jev_ask`)
  - 9/23 오후 MCP 서버가 `HTTP 403: error code: 1010`으로 멈춤 → 파이썬 기본 User-Agent가 막힌 것. User-Agent를 넣어 고침(프롬프트 mcp.2 1차·2차 기록)
  ```bash
...
| 경로 | 내용 |
|---|---|
| `jev-handson/` | **강의 실습**(9/23 녹화): `example.js` · `three-examples.js` · `benchmark-site/`(RouteLab). `env.txt`는 직접 만들기(`.gitignore`에 있음) |
